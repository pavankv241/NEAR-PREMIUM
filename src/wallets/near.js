...
export class Wallet {
  ...
  signIn = async () => {
    try {
      ...
      console.warn('Wallet sign-in error:', error);
      // Fallback: try to show modal without specific contract
      try {
        const modal = setupModal(await this.selector, {});
        modal.show();
      } catch (fallbackError) {
        console.error('Wallet sign-in failed:', fallbackError);
        throw new Error('Unable to connect wallet. Please try again or use a different wallet.');
      }
    }
  };
